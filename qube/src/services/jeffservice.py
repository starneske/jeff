...
class jeffService:
    ...
    def find_by_id(self, entity_id):
        ...
        data = jeff.query.get(entity_id)
        if data is None:
            raise jeffServiceError(
                'jeff {} not found'.format(entity_id),
    ...
    def get_all(self):
        list = []
        data = jeff.query.filter(
            jeff.tenantId == self.auth_context.tenant_id)
    ...
    def update(self, model, entity_id):
        ...
        record = jeff.query.get(entity_id)  # jeff is a mongo class
        if record is None:
            raise jeffServiceError(
                'jeff {} not found'.format(entity_id),
                ErrorCodes.NOT_FOUND)
